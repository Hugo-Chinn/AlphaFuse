...
class diffusion():
    def __init__(self, timesteps, beta_start, beta_end, beta_type, w, linespace, lamda):
    ...
    def p_losses(self, denoise_model, target, x_start, h, t, noise=None, loss_type="l2"):
        ...
        #denoise_model.eval()
        #logits = torch.matmul(predicted_x, denoise_model.item_embeddings.weight[:-1].transpose(0, 1))
        #loss_sm = self.ce_loss(logits, target)
        #denoise_model.train()

        #loss = loss_diff + self.lamda * loss_sm 
